...
class _JRCFIB2022M(FlyTemplateBrain):
    """JRCFIB2022M a.k.a. "male CNS" dataset.

    The JRCFIB2022M EM volume contains a whole male CNS (brain + VNC).
    ...
    References
    ----------
    Berg et al., bioRxiv (2025): Sexual dimorphism in the complete connectome of the Drosophila male central nervous system
...
class _JRCFIB2022Mplot(_JRCFIB2022M):
    """JRCFIB2022M a.k.a. "male CNS" dataset.

    The JRCFIB2022Mplot template is a version of the JRCFIB2022M template space
    in which the VNC has been tilted down by ~90 degrees to allow co-visualization
    of the brain and the VNC from a frontal perspective. In addition, it corrects
    for a small tilt of brain + VNC along the anterior-posterior z-axis such that
    ...
    def mesh(self):
        if not hasattr(self, "_mesh_xf"):
            ...
            self._mesh_xf = navis.xform_brain(
                super().mesh, source="JRCFIB2022M", target="JRCFIB2022Mplot"
            )
    ...
    def mesh_brain(self):
        if not hasattr(self, "_mesh_brain_xf"):
            ...
            self._mesh_brain_xf = navis.xform_brain(
                super().mesh_brain, source="JRCFIB2022M", target="JRCFIB2022Mplot"
            )
    ...
    def mesh_vnc(self):
        if not hasattr(self, "_mesh_vnc_xf"):
            ...
            self._mesh_vnc_xf = navis.xform_brain(
                super().mesh_vnc, source="JRCFIB2022M", target="JRCFIB2022Mplot"
            )
        return self._mesh_vnc_xf


JRCFIB2022Mplot = _JRCFIB2022Mplot(**template_meta["JRCFIB2022M"])
...
class _BANC(FlyTemplateBrain):
    """BANC ("Brain and Nerve Cord") dataset.

    The BANC EM volume contains a whole female CNS (brain + VNC). See References for details.

    References
    ----------
    Bates et al., bioRxiv (2025): Distributed control circuits across a brain-and-cord connectome.
    ...
    def mesh_whole_brain(self):
        """On-demand loading of whole brain mesh."""
        if not hasattr(self, "_mesh_whole_brain"):
            fp = os.path.join(mesh_filepath, "BANC_whole_brain.ply")
    ...
    def mesh(self):
        ...
        if not hasattr(self, "_mesh"):
            # Load the raw mesh (voxels)
            fp1 = os.path.join(mesh_filepath, "BANC_brain.ply")
            fp2 = os.path.join(mesh_filepath, "BANC_vnc.ply")
            self._mesh = tm.util.concatenate(tm.load_mesh(fp1), tm.load_mesh(fp2))
        return self._mesh
    ...
    def mesh_brain(self):
        ...
        if not hasattr(self, "_mesh_brain"):
            # Load the raw mesh (voxels)
            fp = os.path.join(mesh_filepath, "BANC_brain.ply")
            self._mesh_brain = tm.load_mesh(fp)
        return self._mesh_brain
    ...
    def mesh_vnc(self):
        ...
        if not hasattr(self, "_mesh_vnc"):
            # Load the raw mesh (voxels)
            fp = os.path.join(mesh_filepath, "BANC_vnc.ply")
            self._mesh_vnc = tm.load_mesh(fp)
        return self._mesh_vnc


BANC = _BANC(**template_meta["BANC"])


class _AEDES(FlyTemplateBrain):
    """AEDES ("Aedes aegypti") brain dataset.

    The AEDES EM volume contains a whole mated female mosquito brain.
    It was generated by group of Wei Lee (Harvard Medical School) and
    is currently being proofread by a consortium.

    """

AEDES = _AEDES(**template_meta["AEDES"])
